...
RCT_EXPORT_METHOD(updateDeviceInformation:(NSString*)action
    ...
    if ([action isEqualToString:@"set"]) {
        ...
        if ([value isKindOfClass:[NSString class]]) {
            NSDate *date = [RNAcc dateFromString:value];
            if (date) {
    ...
    if (items) {
        [items enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
            if ([obj isKindOfClass:[NSString class]]) {
                NSDate *date = [RNAcc dateFromString:obj];
                if (date) {
...
RCT_EXPORT_METHOD(setInAppLocked:(BOOL)enabled) {
...
// TODO To uncomment
RCT_EXPORT_METHOD(setPushLocked:(BOOL)enabled) {
    [Accengage push].suspended = enabled;
}

RCT_EXPORT_METHOD(isPushLocked:(RCTPromiseResolveBlock)resolve rejecter:(RCTPromiseRejectBlock)reject) {
